...
export const getOverviewAnalytics = async (req, res) => {
  try {
    ...
    const [revenue, occupancy, clients, reviews, topServices, employees] = await Promise.all([
      getRevenueAnalytics({ params: { businessId }, query: { range: "month" } }, { json: d => d }),
      getOccupancyAnalytics({ params: { businessId }, query: { date: new Date().toISOString().split("T")[0] } }, { json: d => d }),
      getClientAnalytics({ params: { businessId } }, { json: d => d }),
      getReviewAnalytics({ params: { businessId } }, { json: d => d }),
      getTopServices({ params: { businessId } }, { json: d => d }),
      getEmployeeAnalytics({ params: { businessId }, query: { month: new Date().toISOString().slice(0,7) } }, { json: d => d }),
    ]);
